...
function createRepeatableFilesystemEffectPreparationV0({authorizationPort,rootResolutionPort,toolAuthorityPort,executionLedger}={}){
 ...
 function prepare(req){
  ...
  let t;try{t=toolAuthorityPort({authorizationId:a.authorizationId,rootResolutionId:root.rootResolutionId});}catch(_){return result(OUTCOMES.UNKNOWN,"tool authority unavailable");}
  if(!plain(t)||t.type!=="GT63_REPEATABLE_FILESYSTEM_TOOL_AUTHORITY"||!nonEmpty(t.toolAuthorityId)||t.authorizationId!==a.authorizationId||t.rootResolutionId!==root.rootResolutionId||t.capability!=="CREATE_NEW_FILE"||t.rootPath!==root.rootPath||t.target?.kind!=="RELATIVE_FILE"||t.target.path!==a.target.path||t.toolAuthorityState!=="AUTHORIZED"||t.scope?.operation!=="CREATE_NEW_FILE"||t.scope.rootPath!==root.rootPath||t.scope.relativeTargetPath!==a.target.path||t.scope.overwriteAllowed!==false||t.scope.deleteAllowed!==false||t.scope.renameAllowed!==false||t.scope.arbitraryPathAllowed!==false||t.authority!=="NONE"||t.authorityEffect!=="NONE"||t.effectPerformed!==false)return result(OUTCOMES.NOT_READY,"tool authority does not bind exact authorization/root/scope");
