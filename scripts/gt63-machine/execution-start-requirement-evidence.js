...
const RULESET_VERSION = 'execution-start-requirement-evidence-v0.1.0';
...
function createExecutionStartRequirementEvidence({ requirementSourcePort } = {}) {
  ...
  function resolve(request) {
    ...
    try {
      ...
      return result(OUTCOMES.UNKNOWN, 'execution start requirement source unavailable');
    }

    if (material == null) return result(OUTCOMES.NOT_RESOLVED, 'execution start requirement not found');
    if (!validMaterial(material)) return result(OUTCOMES.UNKNOWN, 'execution start requirement invalid, non-current, stale, or contradictory');
    ...
    if (material.executionTargetRef !== request.executionTargetRef
      ...
      return result(OUTCOMES.NOT_RESOLVED, 'execution start requirement does not match requested start scope');
    }
    ...
    const evidence = Object.freeze({
      executionStartRequirementEvidenceRef: `gt63-evidence:execution-start-requirement:${digest(evidenceMaterial).slice(7)}`,
      ...evidenceMaterial
    });
