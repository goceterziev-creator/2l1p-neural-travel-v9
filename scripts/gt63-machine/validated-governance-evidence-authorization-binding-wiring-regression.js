...
test("same-binding-is-idempotent-by-binding-identity",()=>{const w=make();const a=w.assess(request),b=w.assess(request);assert.equal(a.outcome,OUTCOMES.AUTHORIZED);assert.equal(b.outcome,OUTCOMES.AUTHORIZED);assert.deepEqual(a.binding,b.binding);assert.equal(a.binding.bindingId,b.binding.bindingId);assert.equal(a.reason,null);assert.equal(b.reason,"same authorization binding already accepted");noDownstream(a);noDownstream(b);});
