...
t("numeric-id-mismatch-not-bound",()=>assert.equal(make().assess(identity({githubUserId:1})).outcome,mod.OUTCOMES.NOT_BOUND));
t("login-mismatch-not-bound",()=>assert.equal(make().assess(identity({githubLogin:"other"})).outcome,mod.OUTCOMES.NOT_BOUND));
t("application-account-is-not-governance-principal",()=>assert.equal(make().assess(identity({principalRef:"gt63-runtime-user:USR-ADMIN"})).outcome,mod.OUTCOMES.BOUND));
...
t("all-outcomes-authority-none",()=>{for(const r of [make().assess(identity()),make().assess(identity({githubUserId:1})),make(()=>null).assess(identity())])assert.equal(r.authority,"NONE");});
