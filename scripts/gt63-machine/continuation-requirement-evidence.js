...
const RULESET_VERSION = 'continuation-requirement-evidence-v0.1.0';
...
function validMaterial(material) {
  const fields = [
    'continuationTargetRef','interactionId','fromInteractionRevision','throughInteractionRevision',
...
function createContinuationRequirementEvidence({ requirementSourcePort } = {}) {
  ...
  function resolve(request) {
    const fields = [
      'rulesetVersion','continuationTargetRef','interactionId','interactionRevision',
    ...
    try {
      ...
      return result(OUTCOMES.UNKNOWN, 'continuation requirement source unavailable');
    }

    if (material == null) return result(OUTCOMES.NOT_RESOLVED, 'continuation requirement not found');
    if (!validMaterial(material)) return result(OUTCOMES.UNKNOWN, 'continuation requirement invalid, non-current, stale, or contradictory');

    if (material.continuationTargetRef !== request.continuationTargetRef
      ...
      return result(OUTCOMES.NOT_RESOLVED, 'continuation requirement does not match requested continuation scope');
    }

    const evidenceMaterial = {
      type: 'GT63_CONTINUATION_REQUIREMENT_EVIDENCE',
      schemaVersion: '1.0',
      rulesetVersion: RULESET_VERSION,
    ...
    const evidence = Object.freeze({
      continuationRequirementEvidenceRef: `gt63-evidence:continuation-requirement:${digest(evidenceMaterial).slice(7)}`,
      ...evidenceMaterial
    });
